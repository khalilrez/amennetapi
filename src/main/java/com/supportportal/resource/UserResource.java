...
public class UserResource extends ExceptionHandling {
    ...
    @Autowired
    public UserResource(AuthenticationManager authenticationManager, UserService userService, JWTTokenProvider jwtTokenProvider, EmailService emailService,AgencyRepository agencyRepository) {
    ...
    public ResponseEntity<User> addNewUser(@RequestParam("firstName") String firstName,
        ...
        User newUser = userService.addNewUser(firstName, lastName, username,email, role, Boolean.parseBoolean(isNonLocked), Boolean.parseBoolean(isActive), profileImage,agency);
        return new ResponseEntity<>(newUser, OK);
    }
    ...
    public ResponseEntity<OneTimePassword> verifyOtp(@PathVariable("username")String username) throws MessagingException {
        OneTimePassword otp = new OneTimePassword();
        String code = RandomStringUtils.randomNumeric(5);
        User user = userService.findUserByUsername(username);
        emailService.sendNewOTPEmail(user.getFirstName(),code,user.getEmail());
        otp.setCode(code);
        return new ResponseEntity<>(otp,OK);
    }
